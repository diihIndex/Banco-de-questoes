...
def limpar_coluna(nome):
    """Função para limpar nomes de colunas sem quebrar o código"""
...
with st.expander("🏫 Configuração da Prova", expanded=True):
    ...
    add_cartao = check1.checkbox("Incluir Cartão-Resposta (Colunas)", value=True)
...
if selecao:
    ...
    # Cartão Resposta (Letras dentro, Máx 12 por coluna)
    ...
    st.divider()
    st.download_button("📥 Baixar Arquivo", html_final, "avaliacao.html", "text/html")
    st.components.v1.html(html_final, height=1000, scrolling=True)
else:
    st.info("💡 Escolha as questões para gerar o documento.")
