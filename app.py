...
# --- 3. NAVEGAÇÃO POR ABAS SUPERIORES ---
st.title("📄 Gerador de Avaliações")
aba_gerar, aba_cadastrar = st.tabs(["📋 Gerar Avaliação", "📥 Cadastrar Questão"])

# --- ABA DE CADASTRO ---
with aba_cadastrar:
    st.subheader("Adicionar Nova Questão ao Banco de Dados")
    with st.form("form_novo_cadastro", clear_on_submit=True):
        col1, col2, col3 = st.columns(3)
        disc_cad = col1.selectbox("Disciplina", sorted(df['disciplina'].unique()) if 'disciplina' in df.columns else ["Português", "Matemática"])
        tema_cad = col2.text_input("Conteúdo/Tema")
        dif_cad = col3.select_slider("Nível de Dificuldade", options=["Fácil", "Médio", "Difícil"])
        
        fonte_cad = st.text_input("Fonte (ex: SAEB, Autoria Própria, Concurso X)")
        texto_base_cad = st.text_area("Texto Base (Opcional - Contexto para a questão)")
        comando_cad = st.text_area("Comando/Enunciado da Questão")
        
        col_alt, col_gab = st.columns([3, 1])
        alts_cad = col_alt.text_input("Alternativas (Separe com ponto e vírgula ';')")
        gab_cad = col_gab.text_input("Gabarito (Letra)")
        
        if st.form_submit_button("Confirmar e Salvar Questão"):
            # Lógica de persistência aqui
            st.success("Questão enviada com sucesso para a planilha!")
...
with aba_gerar:
    ...
    with st.expander("🎯 2. Filtros de Seleção", expanded=True):
        ...
        add_gab = check2.checkbox("Incluir Gabarito do Professor", value=True)
    ...
    if selecao:
        ...
        st.info("💡 Selecione questões na aba acima para visualizar a avaliação.")
