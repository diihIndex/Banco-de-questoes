...
<style>
    ...
    /* Bloco único para Texto Base e Comando */
    ...
    .gabarito-table { border-collapse: collapse; width: auto; margin-top: 10px; }
    .gabarito-table th, .gabarito-table td { border: 1px solid black; padding: 8px 15px; text-align: center; font-family: monospace; font-size: 11pt; }
    .gabarito-header-row { background-color: #f2f2f2; font-weight: bold; font-size: 9pt; }
...
# Scripts para LaTeX (MathJax) e Impressão
MATHJAX_AND_PRINT = r"""
...
with aba_gerar:
    with st.expander("🏫 1. Configurações e Logotipos", expanded=True):
        ...
        l_sme, l_esc = col_img1.file_uploader("Logo Esq."), col_img2.file_uploader("Logo Dir.")
    ...
    with st.expander("🎯 2. Filtros", expanded=True):
        ...
        add_cartao = st.checkbox("Cartão-Resposta", value=True)
        add_gab = st.checkbox("Gabarito", value=True)
    ...
    if selecao:
        ...
        for i, row in df_prova.reset_index().iterrows():
            ...
            <div class="quest-box">
                ...
                <div class="texto-comando-container">{t_base}{row["comando"]}</div>
            """
            ...
            if formato == "Objetiva":
                ...
                html_corpo += "<div style='border: 1px dashed #ccc; height: 150px; margin-top: 10px;'></div>"
            html_corpo += "</div>"
        ...
        if add_cartao and formato == "Objetiva":
            ...
            for c in range(0, num_q, 12):
                ...
                cartao_html += '</div>'
            cartao_html += '</div>'
            cartao_html += '<div style="margin-top:40px; border-top: 1.5px solid black; width: 350px; text-align: center; font-size: 9pt; padding-top: 5px;">ASSINATURA DO ESTUDANTE</div>'
            cartao_html += '</div>'
